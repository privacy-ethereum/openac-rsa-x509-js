...
test("certChainRS2048: prove returns a proof and public values", { timeout: 10 * 60 * 1000 }, async () => {
  ...
  const wtns = await calculateWtns();
  proveResult = await prove(CircuitKind.CertChainRs2048, wtns);
...
test("certChainRS2048: verify accepts the proof from prove()", { timeout: 2 * 60 * 1000 }, async () => {
  assert.ok(proveResult, "prove() must have run first");
  const vkBytes = await readFile(join(ASSETS, "cert_chain_rs2048_verifying.key"));
